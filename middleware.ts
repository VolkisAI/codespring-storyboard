...
export default clerkMiddleware(async (auth, req) => {
  ...
  // Password gate for /login and /signup pages
  if (req.nextUrl.pathname.startsWith('/login') || req.nextUrl.pathname.startsWith('/signup')) {
    const cookieName = 'sbpw';
    const hasCookie = req.cookies.get(cookieName)?.value === 'true';
    const storyboardPassword = process.env.STORYBOARD_PASSWORD;

    if (!hasCookie) {
      const pwParam = req.nextUrl.searchParams.get('pw');

      if (pwParam && storyboardPassword && pwParam === storyboardPassword) {
        // Create a clean URL without the password param
