...
import { Home, Database, Sparkles, CreditCard, BookText, Headphones, GraduationCap, Package } from "lucide-react";
...
export default function Sidebar({ profile, userEmail, whopMonthlyPlanId, whopYearlyPlanId }: SidebarProps) {
  ...
  const navItems = [
    ...
    { href: "/dashboard/storyline", icon: <Database size={16} />, label: "Generate Storylines" },
    { href: "/dashboard/library", icon: <BookText size={16} />, label: "Library" },
    { href: "/dashboard/support", icon: <Headphones size={16} />, label: "Support" },
  ];

  const learningNavItems = [
    { href: "/dashboard/courses", icon: <GraduationCap size={16} />, label: "Courses" },
    { href: "/dashboard/content-assets", icon: <Package size={16} />, label: "Content Assets" },
  ];
  ...
  return (
    <>
      ...
      <div className="h-screen w-[60px] md:w-[220px] bg-white/60 backdrop-blur-xl border-r border-white/40 flex flex-col justify-between py-5 relative overflow-hidden">
        ...
        {/* Navigation & Learning Section Wrapper */}
        <div className="flex-1">
          {/* Navigation Items */}
          <nav className="px-3 relative z-10">
                ...
                >
                  <motion.div 
                    className={`flex items-center py-2 px-3 rounded-lg cursor-pointer transition-all ${
                      isActive(item.href) 
                        ? "bg-[#C5F547] text-black shadow-sm" 
          ...
          <div className="px-3 mt-8 relative z-10">
            ...
            <h3 className="hidden md:block text-xs font-semibold text-gray-400 uppercase tracking-wider px-3 mb-3">
              Learning
            </h3>
            <div className="space-y-1.5">
