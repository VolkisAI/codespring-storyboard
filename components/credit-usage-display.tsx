...
export function CreditUsageDisplay() {
  ...
  // Custom progress bar colors using the brand green
  const progressClasses = "h-2.5 rounded-full bg-[#C5F547]";
    ...
    <Card className="border-white/60 bg-white/80 shadow-sm overflow-hidden">
      <CardHeader className="py-3 px-4 pb-2">
        <div className="flex justify-between items-center">
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full ${membershipType === 'Pro' ? 'bg-[#C5F547]/30 text-black' : 'bg-gray-100 text-gray-600'}`}>
            {membershipType} Plan
          </span>
        </div>
      ...
      <CardContent className="p-4 pt-2">
        {loading ? (
          <div className="flex justify-center py-2">
            <RefreshCwIcon className="h-4 w-4 animate-spin text-[#C5F547]" />
          </div>
        ) : (
          <>
      ...
      <CardFooter className="border-t border-gray-100 pt-2 pb-2 px-4 text-xs flex items-center text-gray-600">
        <CalendarIcon className="mr-1.5 h-3 w-3 text-[#C5F547]" />
        <span>
